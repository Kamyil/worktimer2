...
import { type IWorkRecord, globalState } from "./globalState";
  ...
  export function handleTimeInputChange(
    ...
    globalState.update((currentState) => {
      ...
      if (workRecord) {
        workRecord[startOrEnd][hourOrMinute] = Number((event.target as HTMLInputElement).value);
      } else throw Error(`Couldn't find work record with id=${workRecordId}`);
      ...
      currentState.work_records.forEach((workRecord) => {
        newTotalTimeSpentThisDayInMinutes += workRecord.total_time_spent_in_minutes;
      });
  ...
  export function handleNameInputChange(event: Event, workRecordId: number) {
    globalState.update((state) => {
      if (state.work_records.has(workRecordId)) {
        state.work_records.get(workRecordId)!.name = String((event.target as HTMLInputElement).value);
      } else throw Error(`Couldn't find work record with id=${workRecordId}`);
